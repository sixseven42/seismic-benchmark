...
const GROUP_DESCRIPTIONS: Record<string, string> = {
  ...
  'Field Ground-Roll Noise': 'A field seismic benchmark for ground-roll attenuation on real shot gathers with noise strength 1.0. It evaluates coherent noise suppression methods under natural acquisition conditions with actual surface-wave interference.',
  ...
  'AVO Random Noise': 'A suite of synthetic AVO seismic benchmarks with varying random noise types (Gaussian and Poisson) and SNR levels (-5 dB, 0 dB and 5 dB). Each variant provides paired noisy and clean labels for evaluating random noise suppression methods on AVO gathers.',
  ...
  'Common-Receiver Deblending': 'Synthetic common-receiver deblending benchmarks based on the SEG China 3D (SEGC3) geological model. Each variant uses a different T02 blending scenario (mod, simp, comp), providing pseudo-deblended inputs and corresponding labels for evaluating methods that separate overlapping shot or blended arrivals.',
  'AVO Common-Receiver Deblending': 'A 2D marine field common-receiver deblending benchmark using the Mobile AVO Viking Graben Line 12 open-source dataset. The T03 avo_mod scenario evaluates deblending methods on real marine seismic data with overlapping arrivals.',
